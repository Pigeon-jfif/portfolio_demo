## 2026-09-17 / zipcovers1

- Le 228 cover full e 228 miniature locali passano da 456 file WebP pubblicati a 18 pacchetti ZIP_STORED (9 full + 9 thumbs), in blocchi da 50 ID.
- Nuovo loader browser senza dipendenze esterne: scarica un pacchetto solo quando serve, estrae la WebP in memoria e riusa archivio e Blob URL nella sessione.
- I percorsi in `record-covers.js` restano logici e compatibili con la mappa esistente; le cover remote continuano a usare il percorso normale.
- `prepare_cover.py`, check, build e test sono aggiornati per creare e verificare i pacchetti; aggiunto `cover_packs.py` per aggiornamenti manuali dallo staging.
- Le vecchie cartelle `assets/covers/full` e `assets/covers/thumbs` sono rimosse dal payload patchato.
- GitHub Pages funziona normalmente; per le cover ZIP in locale serve HTTP (per esempio `python -m http.server`) anziche' `file://`.

## 2026-09-17 / smooth4

- Quattro animazioni native lineari sincronizzate; niente transform scritti a ogni frame.
- Cover decodificate prima del ricambio, ancore stabili, cambio solo fuori campo.
- Sfumature statiche, coordinate subpixel, fase conservata al resize.
- Landing adattiva senza vuoto prima del footer; nessun contenuto ritagliato.
- Copyright senza link su tutte le pagine; parola collezione senza sottolinea/hover.
- Media ripuliti mantenuti identici; nuova versione URL per CSS e JS.

## 2026-09-17 / Plastic Pizzas: landing e collezione

- Tre cover a scaletta: autoplay, velocita' lineare, 4500 ms per spazio, nessuna sosta al ricambio.
- Rimossa la presentazione a due righe. Caricamento progressivo e riciclo senza buchi.
- Landing dischi.html separata dal catalogo collezione.html; vecchi URL mantenuti.
- Ripristinati navigazione e rimandi pubblici a Plastic Pizzas.
- Nota di ascolto accanto al titolo; statistiche in fondo al catalogo.
- Pacchetto gia' alleggerito: 239 varianti in meno, senza ricompressione.
- Documentati i 32 file Noale/Conegliano mancanti gia' nell'input.
