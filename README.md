...
Il logo `p.` riporta alla Home. Il menu contiene **Raccolte** e **Chi sono**;
Instagram rimane nei contatti e nel footer. **Plastic Pizzas** e' volutamente fuori dalla navigazione visibile: l'accesso resta nascosto nel simbolo `©` del footer.
La Home resta personale e testuale e mantiene solo il riferimento editoriale ai dischi, senza collegamento diretto al catalogo.
...
**Per provarlo: estrai l'intera cartella e apri `index.html` nel browser.**
Non aprire l'HTML da dentro lo ZIP. Non servono account, framework, installazioni,
server applicativi o database. Non c'e' un pannello di amministrazione: si
modificano normali file di testo commentati.

## Plastic Pizzas: partenza rapida

Apri **`dischi.html`** per entrare direttamente nei dischi durante sviluppo/test. Nel sito pubblico l'accesso intenzionalmente discreto e' il simbolo **`©`** nel footer. Non e' necessario pubblicare il sito o installare Node per visitarlo.
...
**Tre dal catalogo** e' un carosello casuale di tre album con copertina. Ogni
2 secondi il disco a sinistra sfuma fuori, centro e destra scalano di uno slot e
una nuova thumb entra da destra. Le copie dello stesso artista/titolo condividono
la stessa identita: dopo essere comparso, un album resta escluso per almeno
**10 nuove estrazioni**. La prossima cover viene precaricata. Il carosello si
ferma quando esce dal viewport, quando apri una scheda disco, su hover/focus,
quando la pagina e' nascosta e con **Riduci movimento** attivo. Una cover che
fallisce il caricamento viene esclusa dalle proposte della sessione.
...
Per cambiare una cover: prepara la versione web in `assets/covers/full` e la miniatura in `thumbs`, poi
compila la voce con quel codice in `record-covers.js`. `tools/prepare_cover.py` genera entrambe in WebP senza fare upscale.

**Copertine locali:** 228 WebP full a risoluzione nativa e 228 miniature WebP, associati a **235 voci** del catalogo. Le copie duplicate dello stesso artista/titolo condividono lo stesso artwork locale ma restano schede distinte. I JPG ricevuti restano sorgenti esterne al payload del sito; non vengono pubblicati. Tra gli artwork remoti precedenti resta attivo soltanto **Trench**; Dookie e The Dark Side Of The Moon ora usano file locali. Nessuna copertina modifica titolo, anno, formato o note del CSV.
...
- `dischi.html`: catalogo completo di Plastic Pizzas, 433 voci nella consegna iniziale.
...
Le raccolte disponibili sono due, con 46 fotografie in totale (37 + 9). Non ci sono album finti, servizi professionali inventati,
